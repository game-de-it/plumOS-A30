...
- [x] stock PPSSPP の `controls.ini` は `L = 10-193`、`R = 10-192`、`Pause = 10-4008`、`Exit App = 10-4`。plumOS ではL2/R2をボタン化したため、PPSSPPメニュー呼び出しは `--shoulder-layout standard` + `Pause = 10-104` を既定にする。Function button は PPSSPP hang 回避のため既定 `none` にする。2026-06-09 に `user` layout ではユーザーがL2として押す物理ボタンが `BTN_TL` として出てメニューが開かないことを再確認し、`standard` + `PLUMOS_A30_DISPLAY_LOGICAL=854x480` で完全復旧をユーザー目視確認した。
...
- [x] PPSSPP のL2メニューは、`--shoulder-layout standard` でユーザーがL2として押す物理 `KEY_E` を SDL `LEFTSTICK` -> PPSSPP `NKCODE_BUTTON_L2` (`mapped_key=104`) に入れ、`controls.ini` の `Pause = 10-104` へ割り当てる構成でユーザー目視確認済み。`--escape-exit` が付いているとPause発火時に即終了してハングに見えるため、launcher既定から外し、必要な場合だけ `PLUMOS_A30_PSP_ESCAPE_EXIT=1` で有効化する。
