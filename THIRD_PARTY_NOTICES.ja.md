...
StockOS 由来 file は plumOS の MIT License の対象外です。Miyoo A30 の original firmware が
必要なユーザーは Miyoo firmware page を参照してください:
...
| --- | --- | --- | --- |
| StockOS SD payload | Miyoo A30 StockOS SD card files。Firmware page: <https://lomiyoo.com/ja/pages/firmware> | vendor/original StockOS の条件に従います。plumOS MIT License の対象外です。 | boot flow、fallback `MainUI.stock`、stock runtime library、`miyoo/`、`Emu/`、`RApp/`、`App/`、`RetroArch/`、`Themes/` などの互換 file。 |
...
| NextCommander | <https://github.com/LoveRetro/NextCommander> | ファイルマネージャー app の upstream/source 参照先です。調査した upstream repository に個別の `LICENSE` file はありませんでした。 | ファイルマネージャー app。 |
...
- StockOS SD payload の参照先として Miyoo firmware page を残す:
  <https://lomiyoo.com/ja/pages/firmware>
- NextCommander の upstream/source 参照先を残す:
