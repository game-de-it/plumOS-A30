...
ただし実機検証で PicoArch 経路の表示、操作、実速度が成立しない core は、この自動追加対象から
除外します。2026-06-16 時点では `tgbdual` が黒画面、`mednafen_pce` が実動作約30fpsのため
除外対象です。
...
- START menu を TOP から閉じる時だけ full scan と TOP reload を行い、追加された ROM を
  FE 再起動なしで反映する
...
- launch profile が `standalone:<emulator>` の場合は、
  ...
  `standalone:easyrpg` は A30 実機 first-pass
  検証済みの候補として扱う
