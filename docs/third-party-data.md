...
FBNeo `fbneo-v0.2.97.44/*/single-rom-label` seed は、local FBNeo console XML DAT のうち
単一ROM entry で、plumOS scraper が実際に CRC 対象にする拡張子だけを使います。libretro base
CRC と重複する row、libretro thumbnail label に一意解決できない row、region や hack/prototype
などの risky tag が衝突する row、ROM chip 分割型 row は入れません。

## Credit

plumOS appreciates the work of the No-Intro, DAT-o-MATIC, NesterJ_AoEX_R3,
FBNeo, libretro-database, and libretro-thumbnails communities. Their databases and
