...
/mnt/SDCARD/plumos/
  ...
  state/frontend/
    ...
    resume-session.json       legacy resume compatibility state
...
詳細な schema は [plumOS frontend theme model](frontend-theme-model.ja.md) に分けます。
...
`rom_cursor_wrap` は ROM browsing のカーソル挙動です。true の場合、ROM list の
先頭で上を押すと末尾へ、末尾で下を押すと先頭へ移動し、Gallery のカーソル移動も
先頭/末尾で折り返します。Text ROM list の左右ページ移動は従来通り端で止まります。

`boot_resume_mode`:

- `off`: 起動時は通常の TOP を表示する
- `on`: `recent.json` の先頭、つまり最後に起動した ROM/launch profile を自動で開く
- `recent`: 起動時に Recent 画面を表示し、ユーザーが履歴から選ぶ
...
Recent は履歴一覧です。UI Settings の「起動時に前回ROMを開く」は Recent の先頭を
前回 ROM として扱います。plumOS は起動時再開のために save state やゲーム内 save を
作成・読み込みしません。
...
- 履歴から開く場合は、現在の system default ではなく recent に保存された
  `launch_profile` を優先する
- `resume-session.json` は旧 power/resume flow の互換状態として残すが、
  新しい起動時動作では使用しない
- `boot_resume_mode=on` は Recent の先頭を直接起動する。text UI の `boot` は default
  では判断と launch plan だけを表示し、`boot --execute` の時だけ実行する
- `boot_resume_mode=recent` は Recent 画面を表示する
...
- launch profile が `standalone:<emulator>` の場合は、
  ...
  検証済みの候補として扱う
- ROM list で A を押すと `plumos-text-ui launch ... --execute` を呼び、recent に
  最後に起動した ROM/launch profile を記録する
- RetroArch の Auto Save State / Auto Load State は plumOS の電源メニューや起動時動作とは
...
電源ボタン短押しで開く Power menu は、Sleep / Shutdown / Cancel だけを扱います。
ゲーム内 save、save state、SRAM flush などのデータ保全はユーザー責任とし、plumOS は
電源操作前の保存処理を実行しません。
