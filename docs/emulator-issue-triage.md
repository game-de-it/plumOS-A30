...
| PPSSPP movie playback | `standalone:ppsspp` | A user report for SD Gundam G Generation World pointed at missing/incomplete PSP movie playback. The plumOS PPSSPP recipe had forced `USE_FFMPEG=OFF`, while upstream's media engine and PMF player are FFmpeg-backed. PPSSPP now builds with `PPSSPP_USE_FFMPEG=ON` by default and uses the bundled Linux armv7 FFmpeg libraries. The FFmpeg-enabled build also needs a newer `SDL2_ttf` symbol (`TTF_GlyphIsProvided32`), so the launcher keeps the stock SDL2 video path but prepends a PPSSPP-local `libSDL2_ttf` directory before `/mnt/SDCARD/miyoo/lib`. |
