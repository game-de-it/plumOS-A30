...
| PicoArch menu duplicate input | PicoArch menu の A/B/十字キー | A30 では物理 key と `plumos-joystickd` の virtual gamepad の両経路から同じ menu bit が届くことがあり、PicoArch menu で連続入力のように見える場合があった。`in_menu_wait()` で menu state が変化しない重複 event と key-up event を操作として返さず、A/B/Menu は押しっぱなし repeat から外し、方向/L/R だけ遅めに repeat するようにした。既定は initial 650ms / repeat 260ms、`PLUMOS_PICOARCH_MENU_REPEAT_*` で調整可能。2026-06-20 に A30 実機、runtime package、sdroot package の PicoArch binary SHA `135baed8a3a8f32432b1a6ac5e8d4314263ec7b5fbe2fa188a0eed1b3005e550` 一致を確認。 |
