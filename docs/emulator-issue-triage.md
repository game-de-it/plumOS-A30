...
| PicoArch menu duplicate input | PicoArch menu の A/B/十字キー | A30 では物理 key と `plumos-joystickd` の virtual gamepad の両経路から同じ menu bit が届くことがあり、PicoArch menu で連続入力のように見える場合があった。`in_menu_wait()` で menu state が変化しない重複 event を消費し、A/B/Menu は押しっぱなし repeat から外し、方向/L/R だけ遅めに repeat するようにした。既定は initial 650ms / repeat 260ms、`PLUMOS_PICOARCH_MENU_REPEAT_*` で調整可能。2026-06-20 に A30 実機、runtime package、sdroot package の PicoArch binary SHA `27701fe6b61214ce02166cb1fdaaf24c164fafa0dc93865d3b2c2430b36a3849` 一致を確認。 |
