...
| PicoArch menu duplicate input | PicoArch menu の A/B/十字キー | A30 では物理 key と `plumos-joystickd` の virtual gamepad の両経路から同じ menu bit が届くことがあり、PicoArch menu で連続入力のように見える場合があった。`in_menu_wait()` で menu state が変化しない重複 event と key-up event を操作として返さない対策に加え、PicoArch launcher は既定で joystickd に `--no-buttons` を付け、仮想 gamepad は analog axis 専用にした。A/B/Menu は押しっぱなし repeat から外し、方向/L/R だけ遅めに repeat する。`PLUMOS_PICOARCH_JOYSTICKD_BUTTONS=1` で検証用にボタン転送を戻せる。2026-06-20 に A30 実機へ launcher を反映。 |
