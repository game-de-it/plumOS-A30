...
recipe の対応は [libretro core version inventory](libretro-core-version-inventory.ja.md) と
...
PicoArch は RetroArch の代替 frontend として扱い、原則として core build recipe には含めません。
`./scripts/docker-build.sh picoarch` は picoarch 本体を build し、core は
`/mnt/SDCARD/plumos/retroarch/cores` の既存 libretro core を参照します。ただし `fceumm` だけは、
Onion-era の RA 用 core が PicoArch では `retro_load_game` 周辺で止まるため、PicoArch package 側に
互換 core を同梱し、launcher は `picoarch:fceumm` のときだけ
`/mnt/SDCARD/plumos/emulators/picoarch/cores/fceumm_libretro.so` を優先します。picoarch 本体は
...
2026-06-19 の再検証では、A30 Mali presenter 経路の PicoArch からこの互換 core を使うことで、
NES/FDS の `picoarch:fceumm` がどちらも初期画面と 60.099827fps log まで到達しました。
...
`scummvm`、`quasi88`、初期 EasyRPG probe の directory/no-game 系 content 読み込み不備を潰しています。
...
`bluemsx` / `fmsx` も PicoArch の VFS 経由では content load は成功扱いでも MSX BASIC へ落ちる
ことがあるため、VFS を渡さず core 内蔵の標準 file I/O に戻します。2026-06-17 の実機確認で
MSX PICO の `bluemsx` / `fmsx` はどちらも起動OKです。
...
| CD / 3D console | `pcsx_rearmed`, `mednafen_pcfx`, `opera`, `neocd`, `flycast`, `yabasanshiro`, `beetle_saturn`, `mupen64plus_next`, `parallel_n64` | PS1 / PC-FX / 3DO / Neo Geo CD / Dreamcast / NAOMI / Saturn / 64DD などの BIOS を `/mnt/SDCARD/Bios` から読む。`neocd` は `/mnt/SDCARD/Bios/neocd` 配下の `.bin` / `.rom` を中身で判定する。Atari Jaguar / `virtualjaguar` は A30 では性能不足のため通常候補から外す。 |
| handheld / 8-bit | `gpsp`, `mgba`, `mednafen_gba`, `meteor`, `vba_next`, `vbam`, `gambatte`, `gearboy`, `gearsystem`, `mednafen_lynx`, `handy`, `mednafen_ngp`, `mednafen_wswan`, `pokemini`, `freechaf`, `atari800`, `prosystem`, `freeintv`, `o2em` | optional/required BIOS を `/mnt/SDCARD/Bios` から読む。FreeChaF は `sl31254.bin` と、`sl31253.bin` または `sl90025.bin` が必須。 |
| arcade BIOS pack | `fbneo`, `fbalpha2012`, `fbalpha2012_cps1`, `fbalpha2012_cps2`, `fbalpha2012_neogeo`, `mame2000`, `mame2003_plus` | `fbneo/neogeo.zip` などの arcade BIOS/data pack を `/mnt/SDCARD/Bios` 基準で探す。FBNeo Neo Geo cartridge は `/mnt/SDCARD/Bios/fbneo/neogeo.zip` または `/mnt/SDCARD/Bios/neogeo.zip` を使う。 |
| computer / data pack | `bluemsx`, `fmsx`, `puae`, `np2kai`, `nekop2`, `px68k`, `hatari`, `cap32`, `x1`, `bk`, `mu`, `vice_x64`, `vice_xvic`, `fuse`, `squirreljme`, `ecwolf`, `dosbox_pure`, `prboom` | system ROM / machine database / runtime data を `/mnt/SDCARD/Bios` 基準で探す。`np2kai` は `/mnt/SDCARD/Bios/np2kai` 配下の `bios.rom`, `font.rom` or `font.bmp`, `itf.rom`, `sound.rom` を探す。`nekop2` は RetroArch system directory に `np2` を付けた `/mnt/SDCARD/Bios/np2` 配下で `bios.rom`, `FONT.ROM` or `font.bmp`, `itf.rom`, `sound.rom`, `bios9821.rom` を探す。 |
...
PCSX-ReARMed standalone は OSS 音声を含めず、ALSA `default` を選ぶようにします。
これにより PS1 standalone 経路も plumOS の `Soft Volume Master` を通り、ゲーム中の
物理音量 hotkey が効く状態になります。
...
| PCSX-ReARMed r26l | PS1 system の standalone default 候補 | native fb32 rotation、menu 640x480 landscape virtual、Function menu open/return、shadow clear、入力/音/ゲーム画面、ALSA softvol 経由の音量 hotkey OK。 |
...
設定 inventory は [Red Viper A30 設定](red-viper-a30-settings.ja.md) を正とします。
...
| GBA | `gpsp`, `mgba` | `Emu/GBA`, `RApp/gpsp`, `RApp/mgba` | `gpsp` performance first、`mgba` accuracy fallback。`mednafen_gba` / `meteor` / `vba_next` / `vbam` は HW 表示経路でも重いため通常候補から外す。 |
...
| Neo Geo cartridge | `fbneo`, `fbalpha2012`, `fbalpha2012_neogeo` | `Emu/NEOGEO`, `RApp/fbneo` | RA/PICO とも実機確認済み。ゲーム側 ROM set は core の DAT と一致する必要がある。`mame2003_plus` は Neo Geo 通常候補から外す。 |
| Neo Geo CD | `neocd` | `RApp/neocd` | RA/PICO とも実機確認済み。`neocd` は MP3 CUE 非対応のため、WAV/FLAC/OGG/CHD など互換 media を使う。FBNeo Neo Geo CD 経路は通常候補から外す。 |
| Arcade 2D | `fbneo`, `fbalpha2012`, `mame2003-plus` | `Emu/ARCADE`, `Emu/Shoot`, `RApp/fbneo`, `RApp/mame2003_plus` | CPS1/CPS2/Neo Geo/古めのMAMEを主対象にする。Arcade の PicoArch companion は `fbneo` / `fbalpha2012` / `mame2000` / `mame2003_plus` とも通常候補から外し、RA route を使う。推測として、PicoArch は通常軽いが、Arcade core では RetroArch の成熟した video/audio sync、core option、buffer handling のほうが A30 上で frame pacing と音声 underrun に強い。 |
...
| Intellivision | `retroarch:freeintv` | installed `freeintv` | RA は実機確認済み。PicoArch `freeintv` は除外。 |
| Game & Watch | `gw` | backup `gw` | 低負荷。 |
| Pokemon Mini | `pokemini` | backup `pokemini` | 低負荷。 |
| Doom / WAD | `prboom` | `RApp/prboom` | 実用候補。 |
| PICO-8 carts | `fake08` | installed `fake08` | active launch は `picoarch:fake08`; `retro8` は BGM 挙動が不安定なため通常候補から外す。 |
| TIC-80 | `tic80` | backup `tic80` | stock top-levelではないが実用候補。 |
| ScummVM | `standalone:scummvm`, `picoarch:scummvm` | installed core | standalone は回転/マウス/theme 調整済みで初期default候補。PicoArch も analog axis 補正後に実機OK。RetroArch ScummVM は HW 表示経路でも重いため通常候補から外す。 |
| EasyRPG | `standalone:easyrpg` | `RApp/easyrpg` | standalone のみを通常候補にする。libretro RA/PICO は廃止。 |
| OpenBOR | `standalone:openbor` | upstream OpenBOR engine | v6391 を A30 armhf 向けに build し、FE から選択した `.pak` を直接起動する。SDL は plumOS 側を入力/音声用に使い、画面は OpenBOR の完成フレームを fbdev + Mali EGL presenter へ直送して最終回転/fit を行う。 |
...
| PC-88 / PC-98 | `quasi88`, `np2kai` | CPU負荷より入力/keyboard UXが課題。PC-98は RA `np2kai` に `np2kai_joymode=Arrows` を付与する。`np2kai` は `Keypad 3button` で Desire.hdi の hit-any-key 通過をユーザー確認済み。RA `nekop2` は入力問題が残るため通常候補から外す。PicoArch の `quasi88` は色差、`np2kai` / `nekop2` は 60fps 未達のため FE 候補から外す。PC-98文字表示には各coreのBIOS/font directoryへ `FONT.ROM` 相当を置く必要がある。 |
...
| 3DO | `opera` | RA/PICO とも HW 表示経路で性能不足を確認済み。通常候補から外す。 |
| Atari Jaguar | `virtualjaguar` | RA/PICO とも HW 表示経路で性能不足を確認済み。通常候補から外す。 |
| MicroW8 | `uw8` | RA/PICO とも HW 表示経路で 10fps 前後に留まるため通常候補から外す。 |
| PC-FX | `mednafen_pcfx` | RA/PICO とも HW 表示経路で性能不足を確認済み。通常候補から外す。 |
| Uzebox | `uzem` | RA/PICO とも HW 表示経路で性能不足を確認済み。通常候補から外す。 |
...
- PPSSPP/ScummVM/EasyRPG/PCSX-ReARMed standalone package は A30 first-pass validation 済み。ScummVM directory ROM は `.plumos-scummvm-target`、`scummvm-target.txt`、`.scummvm`、隣接 `.scummvm`/`.svm` sidecar から target id を解決し、無い場合だけ `sky` に fallback する。PCSX-ReARMed standalone は `/dev/dsp` が plumOS ALSA softvol を迂回するため、OSS output を使わない。DOSBox Staging standalone は検証済みだが通常対象外。
