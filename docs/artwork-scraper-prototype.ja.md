...
```text
/mnt/SDCARD/Images/<ROM directory alias>/<ROM alias root からの relative stem>.png
```
...
- ROM ごとにまず frontend と同じ lookup 順で `/mnt/SDCARD/Images/<ROM directory alias>` を見る。
...
ユーザーが `/mnt/SDCARD/Images/<ROM directory alias>/...` に置いた thumbnail は通常通り表示します。

通常 scraper は、単純な ROM payload CRC で libretro database と照合しやすい cartridge 系を
対象にします。ユーザー向けの最新対応表は [ROM サムネイルスクレイピング](user/scraping.ja.md) に
あります。以下は設計メモ用の簡易 snapshot です。
...
CRC workers と download workers は system ごとに override できます。未検証の
simple ROM CRC 系 system は `crc_workers=8/8/8`, `download_workers=6/6/6` を基本にし、
実機 benchmark で遅くなる system だけ system 別例外として下げます。
2026-06-21 時点では GBA が `4/4` の例外です。

FE から scraping を実行する間は、通常 FE の省電力設定を継承せず、一時的に
CPU を `1200 MHz`、`4 cores` に固定します。scraping 終了後は開始前の CPU policy に
復元します。`scripts/benchmark-a30-thumbnail-scraper.sh` も同じ条件で測定します。

`package/frontend/plumos/config/frontend/systems.json` の `scraper` field を scraper 対象可否の
正本にします。対象 system は `enabled=true`, `reason=simple_rom_crc` とし、未検証 system は
原則 `crc_workers=8/8/8`, `download_workers=6/6/6` にします。実機 benchmark で遅くなる
system は system ごとの例外として下げます。2026-06-21 時点では GBA が `4/4` の例外です。
...
`existing_thumbnails` は frontend と同じ lookup 順で `/mnt/SDCARD/Images/<ROM directory alias>` を見て、
...
`--fetch` は `systems.json` の `scraper.crc_workers.default` と
`scraper.download_workers.default` を実効 worker 数として使います。まず missing thumbnail queue を
...
保存先は `/mnt/SDCARD/Images/<ROM directory alias>/<relative stem>.png` です。
...
source で照合し、保存先は選択中の ROM directory alias に合わせて `/mnt/SDCARD/Images/gbc/<zip stem>.png`
...
`system_id=fds` として扱い、保存先は対応する ROM directory alias に合わせます。`.zip` は最初に見つかった
`.nes` / `.fds` / `.gb` member を一時展開して CRC を取ります。

## 参照元

完全な source table は `package/frontend/plumos/config/frontend/scraper-sources.tsv` にあります。
これは `systems.json` の `scraper.enabled=true` system と一致している必要があります。
ユーザー向けの対応表は、これら 2 つの file を元に
[ROM サムネイルスクレイピング](user/scraping.ja.md) にまとめます。
...
- ダウンロード済み PNG の resize/cache は、原本 `Images/<ROM directory alias>` を壊さず
