...
- plumOS 側のライブラリ、emulator、RetroArch、libretro core は、build 時点で
...
A30 上で安定して動く範囲で新しい upstream を使うことを基本方針にします。

- SDL2、RetroArch、libretro core、standalone emulator、補助 library は、作業時点の
  latest stable release を確認してから build する
...
- RetroArch と core は build 時点で upstream の latest stable を確認し、A30 実機検証で
