...
- libretro core は latest 追従を既定にせず、Onion が採用している core 名、
  source 由来、実績 commit/build recipe を基準にし、plumOS 側で再 build できる
  recipe として管理する
...
libretro core は例外で、Onion で実用実績のある core catalog と version/commit を
優先します。

- SDL2、RetroArch、standalone emulator、補助 library は、作業時点の latest stable
  release を確認してから build する
- libretro core は Onion 採用 core と同じ名前、同じ source 由来、同じ実績 commit または
  Onion builder recipe に寄せる。正確な commit が未確定のものは暫定 recipe として扱い、
  A30 build と実機検証で採用可否を決める
...
- libretro core は Onion の採用 catalog と実績 version/commit/build recipe を基準にし、
  latest upstream への一括追従はしない
