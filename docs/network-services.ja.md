...
- Network Settings の `NW Service` サブ項目に `SSH`, `FTP`, `SFTP`, `Samba` を checkbox として表示し、A ボタンで ON/OFF する。
...
自動起動しません。SFTP は SSH に依存するため、SSH を OFF にすると SFTP も OFF になり、
SFTP を ON にすると SSH も再度有効化されます。
...
`SERVICE` は `ssh`, `ftp`, `sftp`, `samba`, `all` を受け付けます。
...
[USB Disk Mode](usb-disk-mode.ja.md) にまとめます。
