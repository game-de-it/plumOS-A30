...
- wrapper は `config/system/settings.json` の `wifi_enabled` を尊重する。
  `false` の場合は Wi-Fi runtime を停止し、起動時の IP 待ちをスキップする。
  それ以外の場合は `plumos-network-control --wifi on` を実行し、既に `wlan0` に
  IP がある場合は wpa_supplicant/udhcpc を停止せず接続を維持し、runtime status と
  SSHだけを確認する。IP が無い場合だけ保存済み Wi-Fi 設定から DHCP/IP取得を試みる
...
wrapper 起動時は `plumos-network-rescue` を実行せず、そのまま通常FEを表示し、
保存済みの `wifi_enabled` 方針に従います。Wi-Fi が無効なら runtime を停止します。
Wi-Fi が有効で既に接続済みの場合はその接続を維持し、未接続の場合だけ保存済み
Wi-Fi 設定での接続を試みます。SSH を含む有効化済み network service は background で
開始します。
