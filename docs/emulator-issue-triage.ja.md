...
| PPSSPP movie playback | `standalone:ppsspp` | SDガンダム Gジェネレーション ワールドで一部 movie が再生できないという報告から、PPSSPP の media engine 構成を確認した。plumOS の PPSSPP recipe は `USE_FFMPEG=OFF` を強制していたが、upstream の MediaEngine と PMF player は FFmpeg-backed である。以後は `PPSSPP_USE_FFMPEG=ON` を既定にし、PPSSPP 同梱の Linux armv7 FFmpeg library を使って build する。FFmpeg 有効版は新しい `SDL2_ttf` symbol (`TTF_GlyphIsProvided32`) も必要とするため、launcher は stock SDL2 video path を維持しつつ、`/mnt/SDCARD/miyoo/lib` より前に PPSSPP 専用 `libSDL2_ttf` directory を追加する。 |
...
| PicoArch menu duplicate input | PicoArch menu の A/B/十字キー、Controller settings の bind 待ち | A30 では物理 key と `plumos-joystickd` の virtual gamepad の両経路から同じ menu bit が届くことがあり、PicoArch menu で連続入力のように見える場合があった。`in_menu_wait()` で menu state が変化しない重複 event と key-up event を操作として返さない対策に加え、PicoArch launcher は既定で joystickd の button forwarding を維持し、`PLUMOS_PICOARCH_SDL_JOYSTICK_ONLY=1` で joystick が存在する場合だけ SDL1 の `sdl:keys` 登録を skip する。これにより入力経路は joystickd virtual gamepad に一本化され、Function/Menu も同じ経路で扱える。A/B/Menu は押しっぱなし repeat から外し、方向/L/R だけ遅めに repeat する。さらに Controller settings では、親メニューで押したA/B/方向入力が残ると最初のUP項目へそのまま入ることがあったため、画面入口とbind待ち前後で入力queueをdrainし、新しい key-down event だけをbind対象にする。`PLUMOS_PICOARCH_JOYSTICKD_BUTTONS=0` と `PLUMOS_PICOARCH_SDL_JOYSTICK_ONLY=0` は入力調査用の escape として残す。2026-06-20 に A30 実機へ反映。 |
...
| PicoArch rotated-axis input | PicoArch 全体 | 初期対応では `picoarch:scummvm`, `picoarch:hatari`, `picoarch:prboom`, `picoarch:dosbox_pure` だけに `axisYR` / `axisXR` を渡していたが、`picoarch:fceumm` でも analog stick の向きが崩れるため、PicoArch launcher 全体の default を `axisYR` / `axisXR` に変更する。PicoArch menu では analog axis を menu direction にしないため、D-pad で操作する。 |
