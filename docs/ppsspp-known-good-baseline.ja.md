...
前提にしています。`user` に切り替えると L/R と L2/R2 の解釈が壊れます。
...
表示されます。A30 SDL build では SDL2_ttf runtime は動作していますが、upstream の
Linux/SDL 経路は fontconfig が無く、fallback font も登録されていない場合、足りない文字を
四角として描画します。

plumOS では、PPSSPP assets を staging する際に、言語選択画面の表示名だけを ASCII の
英語名へ正規化します。選択可能な locale は維持し、user-managed な PPSSPP 設定も
上書きしません。さらに A30 向け PPSSPP patch では
`/mnt/SDCARD/plumos/fonts/default.otf` と
`/mnt/SDCARD/plumos/fonts/cjk-fallback.ttc` を SDL2_ttf の fallback font として登録します。
これにより、日本語 UI 文字列は四角にならずに描画されます。この対応は A30 固定 path の
fallback であり、汎用的な upstream 解決としては fontconfig など platform 側の font 管理が
本筋です。
