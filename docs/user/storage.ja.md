...
| `Saves/` | 互換用/手動用の save directory。全 runtime 共通の既定 save 置き場ではありません |
...
どの directory 名と拡張子を使えるかは [対応システム/エミュレータ一覧](emulators.ja.md) を参照してください。
...
save data の保存場所は runtime ごとに異なります。RetroArch、PicoArch、standalone emulator、
Pyxel はすべて同じ save path を使うわけではありません。

RA、PICO、SA、Pyxel の既定挙動とバックアップすべき path は
[セーブデータとステート](save-data.ja.md) を参照してください。
...
`Saves/` は自分で data を置いた場合、または他 frontend から移行した場合にバックアップしてください。
RetroArch の content-dir save や一部 emulator/project の save は ROM の近くに書かれる場合があるため、
`Roms/` もバックアップ対象に含めてください。
