...
| PC-88 / PC-98 | `quasi88`, `np2kai`, `nekop2` | Input/keyboard UX may be harder than CPU load. RA `np2kai` appends `np2kai_joymode=Arrows`; RA `nekop2` keeps the normal gamepad path and uses a core-side joypad-to-keyboard patch. The user confirmed Desire.hdi passes hit-any-key with `np2kai` set to `Keypad 3button`. PC-98 text rendering needs a `FONT.ROM` equivalent in each core's BIOS/font directory. |
