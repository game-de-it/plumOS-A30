...
| PicoArch | `/mnt/SDCARD/plumos/emulators/picoarch/` | `shauninman/picoarch` commit `802047c` を plumOS 用 `platform=plumos` で build する。libretro core は RetroArch と共有し、FE では `picoarch:<core_id>` profile として扱う。A30 では NES + `fceumm` の FE 経由起動、640x480/16bpp fbcon 表示、stock SDL1 runtime、`plumos-joystickd` keyboard profile を確認済み。 |
...
この構成で NES `picoarch:fceumm` は 2026-06-15 に実機で起動確認済みです。
