...
| handheld / 8-bit | `gpsp`, `mgba`, `mednafen_gba`, `meteor`, `vba_next`, `vbam`, `gambatte`, `gearboy`, `gearsystem`, `mednafen_lynx`, `handy`, `mednafen_ngp`, `mednafen_wswan`, `pokemini`, `freechaf`, `atari800`, `prosystem`, `freeintv`, `o2em` | Read optional/required BIOS files from `/mnt/SDCARD/Bios`. FreeChaF requires `sl31254.bin` and either `sl31253.bin` or `sl90025.bin`. |
...
lives in [Red Viper A30 Settings](red-viper-a30-settings.md). Audio is
...
| Arcade 2D | `fbneo`, `fbalpha2012`, `mame2003-plus` | `Emu/ARCADE`, `Emu/Shoot`, `RApp/fbneo`, `RApp/mame2003_plus` | Focus on CPS1/CPS2/Neo Geo/older MAME. Arcade PicoArch companions for `fbneo`, `fbalpha2012`, `mame2000`, and `mame2003_plus` are removed from normal candidates; use the RA routes. Working hypothesis: PicoArch is usually lighter, but for Arcade cores RetroArch's mature video/audio sync, core-option handling, and buffering are more stable on A30 frame pacing and audio underruns. |
