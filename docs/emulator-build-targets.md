...
追いやすくしました。`tgbdual` は state/config を初期化しても実機 raw framebuffer が黒画面、
`mednafen_pce` は実動作が約30fpsに留まるため、Core Settings の `picoarch:<core>` 自動候補から
除外します。GB/GBC は PicoArch 検証では `gambatte`、`gearboy`、`mgba`、`vbam` を使います。
PC Engine 系の PicoArch 検証は `mednafen_pce_fast` を使います。
