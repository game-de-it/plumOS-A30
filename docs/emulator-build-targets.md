...
| Neo Geo cartridge | `fbneo` | `Emu/NEOGEO`, `RApp/fbneo` | `/mnt/SDCARD/Bios/fbneo/neogeo.zip` 配置後に BIOS 読み込みは確認済み。ゲーム側 ROM set は core の DAT と一致する必要がある。 |
| Neo Geo CD | `fbneo` | `RApp/neocd` | `neocd` は MP3 CUE 非対応のため、ISO+MP3 CUE は FBNeo を default にする。`neocd` を使う場合は `/mnt/SDCARD/Bios/neocd` と WAV/FLAC/OGG/CHD media が必要。 |
