...
この core だけ既定で joystickd の X/Y source を `axisYR`/`axisXR` に差し替えます。2026-06-17 時点では
ユーザー報告で軸入れ替わりを確認しており、この補正後の実機再確認が必要です。
...
| ScummVM | `standalone:scummvm`, optional `retroarch:scummvm` | installed core | standalone は回転/マウス/theme 調整済みで初期default候補。PicoArch companion は analog axis 補正後に再確認する。 |
