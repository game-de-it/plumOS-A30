...
upstream latest stable を初期候補にします。一方で libretro core は latest 追従を
既定にせず、Onion が採用している core 名、source 由来、実績 commit/build recipe に
寄せます。A30 実機で動作異常、性能退行、A30 固有不具合が出た場合は stockOS 側の
version、patch、build option も比較対象にします。
...
| libretro cores | `/mnt/SDCARD/plumos/retroarch/cores/*.so` | stockOS の core 名は参考のみ。現在の recipe は Onion 採用 core 名へ寄せ、plumOS default の Class A/B 40 core と Onion catalog 補完用 Class O を `docker/plumos-toolchain/libretro-core-recipes.tsv` で管理する。A30 実機では `fceumm` と `gambatte` の画面表示、Onion 由来 commit の `mednafen_vb` 性能改善を確認済みで、残りは system ごとの起動/performance/input/audio 検証が必要。 |
...
正本にします。既定の `PLUMOS_CORE_FILTER=plumos` は Class A/B の 40 core を対象にし、
`PLUMOS_CORE_FILTER=onion` または `all` は Onion catalog 補完用 Class O も対象にします。
recipe にあって Onion prebuilt に無い core が出ないことは
`scripts/inventory-onion-libretro-cores.sh` で確認します。
...
この実績は historical baseline として残し、今後の再 build は Onion 寄せ recipe を使います。
...
Virtual Boy の `mednafen_vb`/Beetle VB は、plumOS で build した core では A30 で大きく
処理落ちします。2026-06-12 の実機測定では、1344MHz/4core でも FBO 表示ありで約23fps、
...
ただし、この経路は現時点で A30 menu、A30 input mapping、FE launcher cleanup、
画面回転/fit が未統合のため、標準 profile にはまだしません。
...
| --- | --- | --- | --- |
| FC / NES / FDS | `fceumm`, `nestopia` | `Emu/FC`, `RApp/fceumm`, `RApp/nestopia` | Onion と同じ core 候補へ寄せる。 |
...
| Virtual Boy | `standalone:red_viper`, fallback `retroarch:mednafen_vb` | plumOS build の Beetle VB は約30fps止まりだが、Onion 4.3.1 の Beetle VB core は A30 上で null video/audio 約96fpsを確認。core build 再現が次の候補。Red Viper A30 standalone wrapper は表示/入力/ALSA 音声/終了 cleanup を持つ。 |
...
現行 recipe では Class A/B の 40 core を plumOS default として build します。次は deploy 後の
実機検証を以下の順で進めます。
