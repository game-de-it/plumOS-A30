...
| Red Viper | Virtual Boy 用 experimental standalone 候補 | A30 で ARM dynarec は有効。2026-06-12 の実機 probe では headless 322.86fps、software VIP render 込みでも 1344MHz/4core で 289.00fps、648MHz/2core で 135.13fps。ただし現在の standalone は StockOS 由来の描画経路へ切り替え中で、画面向きが未修正です。VB 標準は `retroarch:mednafen_vb` とし、Red Viper は向き/fit/menu/input/cleanup を直してから再評価します。 |
...
ただし、この経路へ切り替えた standalone Red Viper は現時点で画面の向きが正しくなく、
A30 menu、A30 input mapping、FE launcher cleanup、画面回転/fit も未統合のため、
標準 profile にはしません。
...
| Virtual Boy | `retroarch:mednafen_vb`, optional `standalone:red_viper` | `mednafen_vb` は Onion 由来 commit `162918f` へ pin した plumOS build で Bad Apple の実動作を確認済み。Red Viper は StockOS 由来描画経路の画面向き/fit を直すまで experimental profile として残す。 |
