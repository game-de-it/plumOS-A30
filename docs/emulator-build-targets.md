...
| Red Viper | Virtual Boy 用 experimental standalone 候補 | A30 で ARM dynarec は有効。2026-06-12 の実機 probe では headless 322.86fps、software VIP render 込みでも 1344MHz/4core で 289.00fps、648MHz/2core で 135.13fps。StockOS 由来 SDL2 `mali` + GLES2 経路は `red-viper-sdlgl-a30` として build し、画面向き/fit は GLES 最終 quad で修正済み。ただし Bad Apple の120秒負荷は平均35.18fps止まりのため、VB 標準は `retroarch:mednafen_vb` のままにします。Red Viper は menu/input/cleanup/settings を直してから再評価します。 |
...
解消できたため、Red Viper は標準 profile ではなく standalone 実験候補として扱います。
...
`retroarch:mednafen_vb` を Virtual Boy の default profile にし、`standalone:red_viper` を
実験用 profile として残します。A30 wrapper は 3DS 版 Red Viper の menu UI ではなく、
...
読めることを確認しました。ただし A30 menu、A30 input mapping、FE launcher cleanup、
ユーザー設定接続は未統合のため、標準 profile にはしません。
...
| Virtual Boy | `retroarch:mednafen_vb`, optional `standalone:red_viper` | `mednafen_vb` は Onion 由来 commit `162918f` へ pin した plumOS build で Bad Apple の実動作を確認済み。Red Viper は StockOS 由来描画経路の向き/fit は修正済みだが、性能と menu/input/cleanup/settings が残るため experimental profile として残す。 |
