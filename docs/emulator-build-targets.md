...
| PicoArch | `/mnt/SDCARD/plumos/emulators/picoarch/` | `shauninman/picoarch` commit `802047c` を plumOS 用 `platform=plumos` で build できるが、A30 の通常 FE profile にはしない。stock SDL1 fbcon で 640x480 に mode set すると `/dev/fb0` capture は正常でも実機 LCD では走査方向が崩れ、CPU 回転で 480x640 raw に直す試作は黒画面/数fps級の音切れになった。 |
...
A30 LCD scanout の不整合です。CPU 回転での補正も性能面で成立しなかったため、
2026-06-15 時点では FE の通常 profile から外します。
将来再開する場合は、SDL1 surface を CPU で回すのではなく、A30 Mali/EGL presenter へ
PicoArch の最終 frame を載せる方向で検討します。
