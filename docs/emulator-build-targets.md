...
| PicoArch | `/mnt/SDCARD/plumos/emulators/picoarch/` | `shauninman/picoarch` commit `802047c` を plumOS 用 `platform=plumos` で build し、SDL1 は入力/音声だけに使い、最終 640x480 RGB565 frame を A30 Mali/EGL presenter へ渡す。2026-06-15 に `fceumm` + `いっき` で実機表示の向き/左右反転修正を確認済み。ただし安定評価前のため、A30 の通常 FE profile にはまだ入れない。 |
...
このため 2026-06-15 に PicoArch 用の `plat_a30_mali.c` を追加しました。PicoArch 側は
従来通り software surface へ 640x480 RGB565 で描画し、`SDL_SetVideoMode(640x480)` で
物理 LCD を壊す代わりに、dlopen した stockOS の `/usr/lib/libEGL.so` と
`/usr/lib/libGLESv2.so` で raw panel EGL surface を作り、RGB565 texture として全画面 quad へ
転送します。SDL1 は引き続き audio/input 初期化に使いますが、表示の mode set には使いません。
...
確認 capture と log は
`artifacts/a30-probes/picoarch-a30mali/20260615-031549-fixed/` にあります。
