...
| PC-88 / PC-98 | `quasi88`, `np2kai`, `nekop2` | CPU負荷より入力/keyboard UXが課題。PC-98は RA `np2kai` に `np2kai_joymode=Arrows` を付与し、RA `nekop2` は core 側 joypad-to-keyboard patch で通常 gamepad 経路を維持する。`np2kai` は `Keypad 3button` で Desire.hdi の hit-any-key 通過をユーザー確認済み。PC-98文字表示には各coreのBIOS/font directoryへ `FONT.ROM` 相当を置く必要がある。 |
