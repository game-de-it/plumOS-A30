...
| Neo Geo cartridge | `fbneo`, `mame2003_plus`, `fbalpha2012`, `fbalpha2012_neogeo` | `Emu/NEOGEO`, `RApp/fbneo` | RA/PICO とも実機確認済み。ゲーム側 ROM set は core の DAT と一致する必要がある。 |
| Neo Geo CD | `fbneo`, `neocd` | `RApp/neocd` | RA/PICO とも実機確認済み。`neocd` は MP3 CUE 非対応のため、WAV/FLAC/OGG/CHD など互換 media を使う。ISO+MP3 CUE は FBNeo を default にする。 |
