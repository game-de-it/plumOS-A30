...
SDL2 `mali` + GLES2 描画経路へ切り替え中ですが、画面の向きがまだ正しくありません。
向き/fit/menu/input/FE cleanup が揃うまでは experimental profile として扱います。
...
| Render backend | `RENDERMODE`, `SOFT_FLUSH`, `DOUBLE_BUFFER`, `VSYNC` | fbdev A30 backend は `RM_CPUONLY` 固定。2026-06-13 の stock SDL2 `mali` + GLES2 実験では軽い場面で約50fpsに見える一方、Bad Apple 120秒測定は平均35.18fps、最小14.49fpsでした。現在の StockOS 由来描画経路は画面向きが未修正です。A30 用 HW backend 候補ではありますが、標準 profile にはしません。framebuffer 側 vsync は `Wait Vsync` として接続。 |
...
- `RM_GPUONLY`, `RM_TOGPU`, `RM_TOCPU` は Citro3D/GPU backend 前提です。A30 では stock SDL2 `mali` + GLES2 frontend の実験で一部効果を確認しましたが、Bad Apple の120秒負荷では平均35.18fps止まりで、現在の StockOS 由来描画経路は画面向きも正しくありません。menu/input/FE cleanup/画面回転が統合されるまで、ユーザー向け設定にはしません。
