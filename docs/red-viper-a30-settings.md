...
| Render backend | `RENDERMODE`, `SOFT_FLUSH`, `DOUBLE_BUFFER`, `VSYNC` | 現行 A30 標準 backend は `RM_CPUONLY` 固定。2026-06-13 の stock SDL2 `mali` + GLES2 実験では軽い場面で約50fpsに見える一方、Bad Apple 120秒測定は平均35.18fps、最小14.49fpsでした。A30 用 HW backend 候補ではありますが、Bad Apple のピーク負荷対策としては未解決です。framebuffer 側 vsync は `Wait Vsync` として接続。 |
...
- `RM_GPUONLY`, `RM_TOGPU`, `RM_TOCPU` は Citro3D/GPU backend 前提です。A30 では stock SDL2 `mali` + GLES2 frontend の実験で一部効果を確認しましたが、Bad Apple の120秒負荷では平均35.18fps止まりで、menu/input/FE cleanup/画面回転も未統合のため、まだユーザー向け設定にはしません。
