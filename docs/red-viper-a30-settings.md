...
| Render backend | `RENDERMODE`, `SOFT_FLUSH`, `DOUBLE_BUFFER`, `VSYNC` | 現行 A30 標準 backend は `RM_CPUONLY` 固定。2026-06-13 の stock SDL2 `mali` + GLES2 実験では Bad Apple が約50fpsで表示されたため、A30 用 HW backend 候補として継続調査します。framebuffer 側 vsync は `Wait Vsync` として接続。 |
...
- `RM_GPUONLY`, `RM_TOGPU`, `RM_TOCPU` は Citro3D/GPU backend 前提です。A30 では stock SDL2 `mali` + GLES2 frontend の実験で効果を確認しましたが、menu/input/FE cleanup/画面回転が未統合のため、まだユーザー向け設定にはしません。
