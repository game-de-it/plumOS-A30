...
`plumos-safe-hotkeyd --oneshot` を自動起動し、standalone emulator 中は
`plumos-safe-hotkeyd --volume-only` を自動起動します。`PLUMOS_SAFE_HOTKEYD_AUTOSTART=0`
で無効化できます。自動起動した RetroArch 用 hotkeyd の `SIGUSR1` trigger は
