...
boot wrapper は `/mnt/SDCARD/plumos/bin/plumos-network-rescue` を background で自動実行し、
画面は通常FE TOPへ進めます。network recovery は START menu の `Network Recovery` から
専用画面へ入り、A ボタンで Wi-Fi init script、DHCP、`/mnt/SDCARD/plumos/ssh/start-ssh.sh`
を順に再実行します。Network Settings の `Run Network Recovery` 行も同じ helper を
A ボタンで呼びます。
...
START menu の `Network Recovery` と Settings の network recovery A 実行も確認し、どちらも
