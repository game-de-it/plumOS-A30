...
reboot 復旧用の仮 UI として `--rescue-network` があります。この画面では A ボタンで
`/mnt/SDCARD/plumos/bin/plumos-network-rescue` を実行し、Wi-Fi init script、DHCP、
`/mnt/SDCARD/plumos/ssh/start-ssh.sh` を順に再実行します。boot wrapper はこの helper を
FE 起動時に自動実行し、当面 Mali rescue UI を `plumos-frontend` より前に出します。
START menu の Network と Settings の `A30 Wi-Fi Config`/`A30 Wi-Fi Runtime` も同じ
helper を呼びます。
...
横画面として読めることを確認しました。
`--rescue-network --script a,q` でも、Mali UI から A 相当で network rescue helper を呼び、
exit code `0` で終了することを確認しました。
START menu の Network と Settings の `A30 Wi-Fi Config` A 実行も確認し、どちらも
`udhcpc` lease 取得と SSH start まで到達しました。
ただし `reboot` command は暗転/LED消灯後に Miyoo logo へ進まず、ユーザーが電源長押しで
強制終了してから電源投入しました。起動後は A ボタンの network rescue で SSH が復旧しました。
...
- START menu: Settings/Favorites/Recent は実画面へ遷移し、Network は network rescue を実行する。その他は action preview を出す
- SELECT: system/per-ROM core preview
- Function: SAFE menu を開く。SAFE menu は `Sleep`, `Shutdown`, `Cancel` を持つ
- Settings: 現在値を一覧表示し、Wi-Fi 行の A は network rescue、その他の A は edit preview を出す
